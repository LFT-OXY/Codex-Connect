...
const ENGLISH_RUNNING_DESKTOP_TEXT: RunningDesktopText = RunningDesktopText {
    ...
    content: "The current Codex was not started by Codex Connect. To use Codex Connect, Codex must be restarted.",
    restart: "Exit and restart automatically (Recommended)",
    retry: "I've quit Codex, try again",
    fallback: "The current Codex was not started by Codex Connect. To use Codex Connect, Codex must be restarted.\n\nYes: exit and restart automatically.\nNo: try again after you quit Codex.",
};

const CHINESE_RUNNING_DESKTOP_TEXT: RunningDesktopText = RunningDesktopText {
    instruction: "Codex 已在运行",
    content: "当前 Codex 不是由 Codex Connect 启动的。要使用 Codex Connect，需要重新启动 Codex。",
    restart: "自动退出并重新启动（推荐）",
    retry: "我已退出，重试",
    fallback: "当前 Codex 不是由 Codex Connect 启动的。要使用 Codex Connect，需要重新启动 Codex。\n\n是：自动退出并重新启动。\n否：退出后重试。",
};
...
fn message_box(message: &str, kind: u32) -> i32 {
    ...
    let caption = wide_null("Codex Connect");
...
fn choice_dialog(
    ...
    let title = wide_null("Codex Connect");
