...
- `usage-dashboard.ts#renderLocalUsage(document, result, messages)` 只依赖 `createElement/append/setAttribute/dataset/style`，可用假 DOM 测试；`usage-page.ts` 负责周期按钮、自定义表单、刷新与请求。DOM 标记（测试与 e2e 依赖）：`[data-usage-total]`（title 为完整数）、`[data-usage-segment=<harnessId>]`（style.width 百分比）、`[data-usage-harness-card="all"|<harnessId>]`、`tr[data-usage-day=<date>]`、统计块卡片 `[data-usage-stat="last7Days"|"last30Days"|"dailyAverage"|"conversations"]`（按此顺序，位于 Harness 卡片与每日明细之间，首个子元素 title 为完整数；范围为空时仍显示，`stats.firstActiveDate === null` 时整块省略）、开始使用与活跃天数行 `[data-usage-history]`、`[data-usage-period=<kind>]`（`aria-pressed`）、`form[data-usage-custom]`、`[data-usage-custom-from/to]`、`[data-usage-action="refresh"]`。
