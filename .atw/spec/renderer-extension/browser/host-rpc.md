...
- 较新的方法把常量放在 shared-contracts，由 Renderer 和 Host 共用：`IDLE_RELEASE_SETTINGS_METHOD`、`CREDENTIAL_IMPORTS_METHOD`、`HARNESS_LAUNCH_SETTINGS_GET/SET_METHOD`、`LOADED_SESSIONS_METHOD`。**新方法沿用这种写法**，不要继续在 Renderer 里定义私有常量。
