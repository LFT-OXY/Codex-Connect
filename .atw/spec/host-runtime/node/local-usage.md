...
- Good（Provider）：Pi 在同一周用了 `openai-codex` 6,400 与 `anthropic` 100 → Pi 卡片 `providers` 两项降序；Harness 占比 0.00008% 显示「<0.01%」而不是「0.00%」。
- Base（Provider）：Claude Code 不记录 Provider → `providers: []`，卡片没有展开项；官方 Codex 只有 `openai` → 一个 100% 的子项（用户可改为只在 ≥2 项时展开，需改 Renderer 条件与产品文档）。
...
// Correct：Host 只聚合非 null Provider，Renderer 只看数据
if (harness.providers.length > 0) card.append(providerBreakdown(document, harness, messages));
```
