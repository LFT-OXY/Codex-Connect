...
- 修改 `HarnessAdapter.nativeUsage`、`LOCAL_USAGE_QUERY_METHOD` 的参数/结果、`host-runtime/src/local-usage-*.ts`（含计价与价格来源）、Renderer 用量页，或新增一个 Harness 的原生用量读取时阅读。
...
- 结果（strict）：`range{from,to}`、`totals{total,input,cacheRead,cacheWrite,output,reasoning,conversations}`（refine：`total` = 五项之和）、`estimatedCostUsd`（非负有限数，所选范围的 Estimated Cost，USD）、`models`、`harnesses[{harnessId,name,totalTokens,models}]`（按用量降序，只含 Token > 0 的 Harness）、`daily[{date,total,input,output,cacheRead,reasoning,conversations}]`（有 Token 或对话的日期，倒序）、`stats{last7Days,last30Days,dailyAverage,activeDays,firstActiveDate}`。不含正文、会话 ID、工作目录。
...
- `shared-contracts/test/local-usage.test.ts`：params/结果 schema 边界、`total` refine、`stats` 的两条 `superRefine` 与非整数/非法日期/多余字段，`estimatedCostUsd` 负数/无穷/NaN/字符串/缺失被拒。
