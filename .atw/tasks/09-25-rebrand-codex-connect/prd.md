...
> 2026-09-25 变更：按用户要求，设置页移除"关于"页、导航中的"其他"分组和"Star 支持"链接。上游署名只保留在 README 致谢中，设置页不再出现上游仓库地址。更新页的 Star 提示保留。下面三条是移除前的原始决策。
...
7. **品牌守卫（唯一新增接缝）**：新增一个测试，扫描面向用户的源码（本地化文案、设置页、Harness 安装引导、发布脚本、Rust 平台层与启动器的用户可见输出、README 与简体中文 README），断言：
   - 不含上游仓库地址 `BytePioneer-AI/codex-host`。唯一例外是中英文 README 致谢中的上游署名链接，必须各恰好一处（"关于"页已按用户要求移除）。
