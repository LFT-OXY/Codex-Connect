...
页面上方是按账号分组的额度横条列表，下方是「Pi 中的账号」专区。每个账号一组：组头左侧是产品 Logo 与身份，右侧只放可导入目标的小 Pi 图标；组内每个额度窗口一行。不再有逐行「刷新额度」或「原生管理」入口（全局刷新在工具栏，原生管理边界只以身份的悬停提示保留）。视觉沿用原设置外壳：无边框搜索、全局额度刷新。所有账号组统一使用各自的产品 Logo（Codex 用官方 Codex 图标，不再是自绘的紫色终端方块），Pi 专区中识别出的供应商使用同一份 Logo。工具栏的「账号」数量包含当前 Codex 账号和实际返回的其他 Harness 账号，不随搜索筛选改变。
...
- 每个窗口一行：左侧窗口名（`5 小时`、`7 天`、`周额度`、`月额度`，模型或产品专属窗口写作「名称 · 窗口」，如 `Fable · 7 天`；Grok 产品用量显示 Build、Chat 等），中间横条，右侧百分比与距重置的倒计时。账号级窗口在前，同一模型组的窗口相邻，其余保持来源顺序；只画实际报告的窗口，缺失的窗口不补成已用 0% 或剩余 100%，也不合并或丢弃重复报告。Codex Pro 20x 只显示 7 天窗口，使账号只有一个可比较的额度。
- 默认按「剩余」展示，也可切换为「已用」；横条、百分比与节奏标记同步切换口径，风险颜色仍按已用比例判断：70% 起警示，90% 起强调。
- 节奏标记：窗口长度可确定（5 小时窗口为 5 小时，7 天与周窗口为 7 天；月额度、未知窗口和无法判定长度的产品用量没有）且有重置时间、已用 ≥5% 时，横条上画一根竖线，表示匀速使用时此刻应到的位置（窗口已流逝时长占窗口总长，限制在 0～100%）。实际已用超出该位置 3 个百分点以上时竖线变为警示色；「剩余」口径下竖线随横条镜像。悬停横条可读取节奏说明。
- 倒计时最多两个单位：超过一天为 `6d17h`，不足一天为 `4h54m`，不足一小时为 `14m`。悬停和辅助技术可读取倒计时说明及包含年份、时区的完整本地重置时间。无有效重置时间时不编造日期或倒计时。
- 页面本地每分钟及重新获得焦点时更新倒计时与节奏标记，不重新查询 Host、不重建账号组。到点只显示「待刷新」，不会自动把额度设为 100%；关闭设置后停止计时。
- 窄窗口下窗口名、百分比与倒计时在一行，横条换到下一行，不出现横向滚动。
...
统一列表中展示 Grok Build、agy（Antigravity）、Claude Code 当前原生认证可读取的真实额度。原管理列改为组头的目标 Harness 图标；原生管理边界保留在账号信息的说明中。这不是多账号管理：不提供添加、删除、切换、设为默认或重置卡操作，也不修改 Codex 当前账号。搜索和已用/剩余切换作用于所有账号组，刷新按钮重新查询两类额度。各 Harness 独立并行查询，任一有效结果返回后立即显示，不等待其他 Harness；全局刷新期间同样逐项恢复。
...
- 左侧展示各 Harness/供应商的产品 Logo；主标题优先显示邮箱或可识别名称，Harness 名称和套餐作为次级信息。没有账号身份时以 Harness 名称为主标题，不重复名称或显示「当前登录账号」，不会猜测邮箱。邮箱按可用宽度省略，悬停可查看完整身份。不记录或展示账号快照更新时间；原型中的示例套餐不作为真实数据来源。
...
- 仅 Codex 与 Grok 账号组在组头右侧显示 16px 的 Pi 小图标，其他账号（如 Claude Code）不渲染任何入口。未复制时为弱化单色，已复制时为常规强度并带小圆点（配置已变更为警示色），悬停显示入口名与状态。点击未复制的图标，在单个确认对话框里查看来源、模型入口名称、新增范围与风险；未确认不写入，成功后在同一对话框提示一次“新开的 Pi 会话即可选用，无需重启；已打开的会话没看到时重新打开即可”。点击已复制的图标会定位到下方对应记录。默认入口名让人能看出账号：某个 Provider 的第一个账号用 `codex` / `grok`，之后的账号用 `codex-<邮箱前缀>`（如 `codex-alice/…`），仍冲突或没有邮箱时用 `codex2`、`codex3`…；Pi 里同一 Provider 可以并存任意多个账号，各占一个入口。名称须为 1–48 位小写字母、数字或连字符，以字母开头。
...
- `packages/renderer-extension/src/settings/accounts.css`：工具栏、身份、重置卡与 Pi 专区的遗留样式；额度横条使用设置页 Tailwind 工具类。
