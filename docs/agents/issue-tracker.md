...
This repo tracks work in the ATW task system. All issues — the spec and the implementation tickets for the task you are working on — live as markdown files in the current ATW task directory. There is no remote mirror: the task directory is the **sole authority**, and nothing is synced anywhere. The GitHub repo (`LFT-OXY/Codex-Connect`) has Issues disabled, so `gh issue ...` commands fail here — don't try them.
...
docs/project/领域术语表.md  domain glossary        atw-domain-modeling
...
First enable Issues on the GitHub repo (Settings → Features → Issues); with Issues disabled, every sync call fails.
