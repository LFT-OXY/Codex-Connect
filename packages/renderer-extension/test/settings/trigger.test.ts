...
describe("Renderer settings navigation rail trigger", () => {
  it("badges the icon for updates and opens the Updates page directly", () => {
    ...
    try {
      ...
      expect(button.children[0]?.tagName).toBe("IMG");
      ...
      expect(control.button.title).toBe("Codex Connect settings · A new version is available.");
      ...
      expect(badge?.style.display).toBe("none");
      expect(control.button.title).toBe("Codex Connect settings");
      control.dispose();
    } finally {
      vi.unstubAllGlobals();
    }
